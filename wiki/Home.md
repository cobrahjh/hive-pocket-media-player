# Hive Pocket Media Player

**https://pocket.kinghive.online**

A music player with live visuals that runs entirely on your phone. No account, no server, and no
network at all after the first load.

| | |
|---|---|
| **[Install it](Install)** | Put it on your home screen, and what to do if you had the old address |
| **[Using it](Using-it)** | The microphone, your finger, and what every control does |
| **[Your music folder](Your-music-folder)** | Why it asks once after a cold start, and what the app can and cannot remember |
| **[Privacy](Privacy)** | What leaves the device: nothing |
| **[For developers](For-developers)** | Repo layout, the test suites, and how a release ships |

## In one paragraph

Point it at music on the phone and it plays, drawing an equalizer and effects from that audio.
Point the **microphone** at the room and it draws whatever it can hear, including music playing
from a completely different app. Touch the visuals and they answer: drag to paint, two fingers to
send a pair of wisps wandering. Tap for full screen.

Everything here is drawn from sound the app can actually read, which is why the microphone
matters: it is the one source that can see audio this app does not own.
